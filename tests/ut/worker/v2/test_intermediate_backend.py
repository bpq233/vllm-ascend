...
def backend():
    ...
    cache_cls = load_class("intermediate_cache.py", "IntermediateKVCache", dict(OrderedDict=OrderedDict))
...
def test_cached_rounds_forward_only_suffix_and_hydrate_secondary(backend):
    ...
    assert obj.forward_tokens == 5  # Only the last hidden-state row is needed.
    assert obj.model.call_args.kwargs["positions"].tolist() == [3]
    ...
    assert obj.forward_tokens == 8  # Full-prefix implementation would execute 15.
    assert obj.reused_tokens == 7
...
def test_host_history_delta_reorder_growth_and_slot_reuse():
    cls = load_class("multi_stage.py", "MultiStageDFlashSpeculator", dict(torch=torch, AscendDFlashSpeculator=object))
    obj = cls.__new__(cls)

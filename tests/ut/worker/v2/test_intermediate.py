...
def test_sparse_capture_sizes_cover_verifier_and_secondary(modules):
    ...
    assert sizes == [1, 16, 32, 48, 64, 256]
    # Long prefixes stay in the eager path; they must not create giant graphs.
    assert config.intermediate_capture_sizes(40960, 4, 15) == [1, 16, 32, 48, 64, 256]
    assert config.intermediate_capture_sizes(40960, 4, 15, [64, 256]) == [64, 256]
    assert config.intermediate_capture_sizes(4096, 4, 15, [256, 64, 256]) == [64, 256]

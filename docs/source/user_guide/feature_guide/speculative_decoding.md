...
repeatable layer experiments. The initial concrete adapter supports dense
Qwen3 (including Qwen3-8B) and Qwen2 decoder layers; unsupported architectures,
pipeline parallelism, quantization, LoRA, or KV transfer fail open by default.
...
validity. When false, q' runs a complete no-cache forward for each draft block;
